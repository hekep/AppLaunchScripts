...
| `apps[].name`      | Application name, as used by `hs.application.launchOrFocus()` (`"Chrome"` is accepted for `"Google Chrome"`). Optional when `url` is present — then it is only the installed-check |
| `apps[].url`       | Application hammerspoon url, to launch the application with a predefined state (e.g. `hammerspoon://launchclaudecode<session>` — any address from the generated catalogs works, see `help()`). **`url` overrides `name`** for launching; when `name` is also given it is used to check that the application is installed — **if the app is not installed, the url is not launched** — and to place the window in the layout. Without `name` the entry is launch-only: no layout slot, no window management. Since 0.11.0 — see [WorkSpaceUseCaseWithApplicationState.md](WorkSpaceUseCaseWithApplicationState.md) |
